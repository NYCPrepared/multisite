	...
	//Gets the table of all networks - used on the site admin - blog networks page
	 function get_networks_table() {
    	...
    	<style type="text/css">
    		...
    		table.wp-list-table tbody tr td.fields {
    			padding: 0;
    		}
    	</style>

    	  <h3>Update Networks</h3>
		  <table class="wp-list-table widefat fixed">
		    <thead>
		      <tr>
		        <th class="network-id">ID</th>
		        <th class="network-name">Name</th>
		        <th class="network-slug">Slug</th>
		        <th class="network-description">Description</th>
		    ...
		    <tbody>
		      ...
		      <tr>
		        <td colspan="5" class="fields">
		          ...
		          <table>
		              <tr>
		                <td id="" class="network-id"><?php echo $network->id; ?></td>
		                <td class="network-name"><input type="text" maxlength="140" name="network" value="<?php echo $network->network_name; ?>"></td>
		                <td class="network-slug"><input type="text" maxlength="140" name="slug" value="<?php echo $network->slug; ?>"></td>
		                <td class="network-description" rowspan="2"><textarea name="description" cols="40" rows="5" maxlength="1000"><?php echo $network->description; ?></textarea></td>
		                <td class="actions" rowspan="2">
		                  <input type="hidden" name="network_id" value="<?php echo $network->id; ?>">
		                  <input type="hidden" name="action" value="edit">
		                  <input type="submit" class="button-primary save" name="edit" value="Update"></td>
		              </tr>
		              <tr>
		                <td class="network-banner" colspan="2"><label for="banner-<?php echo $network->id; ?>">Banner Image</label><input type="url" id="banner-<?php echo $network->id; ?>" maxlength="140" name="banner" value="<?php echo $network->banner; ?>">
		                </td>
		                <td class="network-thumbnail network-slug"><label for="thumbnail-<?php echo $network->id; ?>">Thumbnail Image</label><input type="url" maxlength="140" name="thumbnail" id="thumbnail-<?php echo $network->id; ?>" value="<?php echo $network->thumbnail; ?>">
		                </td>
		              </tr>
		          </table>
		          </form>
		        ...
		        <td class="delete">
		          <?php if ($network->id != 1) { ?>
		          <form name="deletecat" method="post" onsubmit="return confirm("Are you sure you want to delete this network?");">
		            <input type="hidden" name="action" value="delete">
		            <input type="hidden" name="networkid" value="2">
		            <input type="submit" class="button submitdelete deletion" value="Delete">
		          </form>
		          <?php } else { ?>
		            Network 1 can not be deleted.
		          <?php } ?>
		        </td>
		      </tr>
		      <?php } ?>
		      ...
		      <tbody>
		        <tr>
		          <td class="network-name">
		          	<input type="text" maxlength="140" name="network" value="">
		            <input type="hidden" name="action" value="add"></td>
		          <td class="network-slug"><input type="text" maxlength="140" name="slug" value=""></td>
		          <td class="network-description" rowspan="2"><textarea name="description" cols="40" rows="5" maxlength="1000"></textarea></td>
		          <td class="actions" rowspan="2"><input type="submit" class="button button-primary add new" value="Add"></td>
		        </tr>
		        <tr>
		          <td class="network-banner"><label for="banner">Banner Image</label><input type="url" maxlength="140" id="banner" name="banner" value=""></td>
		          <td class="network-thumbnail network-slug"><label for="thumbnail">Thumbnail Image</label><input type="url" maxlength="140" id="thumbnail" name="thumbnail" value=""></td>
		        </tr>
		      </tbody>
		    </table>
		  </form>
    	
    	<!-- echo "<table class='wp-list-table widefat fixed'><thead><tr><th>ID</th><th>Name</th><th>Slug</th><th>Description</th></tr></thead>";
    	foreach ( $this->get_networks() as $network )  {
			
    		echo "<tbody><tr><form name='catupdate' method='post'><td align='center'>" . $network->id . "</td>";
			echo("<td><input type='text' maxlength='140' name='network' value='" . $network->network_name . "'><br />140 chars max");
			echo("<td><input type='text' maxlength='140' name='slug' value='" . $network->slug . "'><br />140 chars max");
			echo("<td><textarea name='description' cols='40' rows='5'>" . $network->description . "</textarea><br />8000 chars max" );
			echo("<input type='hidden' name='network_id' value='" . $network->id . "'>");
			echo("<td><input type='hidden' name='action' value='edit'> <input type='submit' class='button' name='edit' value='Update'> </td>");
			echo("</form><td>");
			if ($network->id != 1) {
    	...
    	echo("<tr><form name='catadd' method='post'><td>&nbsp;</td><td><input type='text' maxlength='140' name='network' value=''><br />140 chars max<input type='hidden' name='action' value='add'></td>");
